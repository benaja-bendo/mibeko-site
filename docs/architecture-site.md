...
L'image de partage par défaut (`public/og-default.png`, 1200×630) est déclarée par `Layout.astro` en URL absolue avec ses dimensions ; les pages peuvent la remplacer via la prop `image` (les couvertures de guides/démarches passent déjà une URL absolue).
