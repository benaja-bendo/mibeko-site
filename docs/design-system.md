...
| Écran de l'application, rejoué | `AssistantConversation.astro` | **Reproduction de l'écran Assistant de `mibeko-app-kmp`** (`ChatScreen.kt`), animé en boucle : frappe de la question dans le champ, envoi, bulle utilisateur, pastille « Mibeko IA », trois points d'attente, phrase de sources, cartes d'article. Décision du 27/08/2026. Trois obligations. ① **La géométrie de l'application vit dans le `<style>` du composant, jamais en classes utilitaires** — bulles à 16 dp, champs à 4 dp, pastilles rondes : c'est le vocabulaire de l'app, il ne doit pas contaminer celui du site (les cinq greps du § 2 restent donc à zéro). ② **Deux traductions volontaires et bornées** : l'`elevation 2dp` des cartes devient un filet de 1 px (la profondeur se fait par le trait), et le fil s'ancre en bas alors que l'application empile depuis le haut (sinon quatre cents pixels de vide pendant la frappe). ③ **Surcouche** : le serveur rend la conversation terminée et lisible ; sans JavaScript, avec `prefers-reduced-motion` ou pour un robot, c'est cet écran-là qui reste. La boucle se met en pause hors écran et onglet masqué (§ 11). **On y reproduit le thème du mobile, pas celui du dashboard** : « Lex Gold » est la marque abandonnée, le thème mobile porte en commentaire « parité web » et emploie exactement les jetons de ce site. |
