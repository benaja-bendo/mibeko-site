...
## Règle absolue
**Pas de compte utilisateur sur `mibeko.fr`.** Ni login, ni inscription, ni état utilisateur persistant, ni IA conversationnelle. Tout ce qui exige une authentification vit sur `app.mibeko.fr` (le site n'y renvoie que par **liens sortants** : en-tête « Se connecter » / « Créer un compte » depuis le 07/09/2026, `/assistant`, `/professionnels`, `/application`, `/contact`). L'en-tête porte ces deux liens sur toutes les pages parce que l'unique porte vers un compte était auparavant le lien « Professionnels » — un étudiant ou un citoyen devait se déclarer professionnel pour créer le sien. Ce sont des `<a href>` vers un autre domaine : aucun formulaire d'authentification, aucun état utilisateur, la règle absolue tient. Les seuls `POST` du site sont `src/pages/api/contact.ts` et `api/newsletter.ts`, qui relaient vers l'API Laravel en serveur-à-serveur.
...
L'accueil montre en plus l'**écran Assistant de l'application mobile rejoué en boucle** (`AssistantConversation.astro`, décision du 27/08/2026) : ce n'est pas une interface de chat générique mais la reproduction de `ChatScreen.kt` de `mibeko-app-kmp`, dont le thème est en parité de jetons avec ce site. Trois invariants : la géométrie de l'app (bulles 16 dp, champs 4 dp, pastilles rondes) reste **confinée au `<style>` du composant** — jamais en classes utilitaires, les greps du § 2 de la charte doivent rester à zéro ; le serveur rend la **conversation terminée**, si bien que sans JS, en `prefers-reduced-motion` ou pour un robot on voit un écran complet et lisible ; et **aucun texte de réponse n'est inventé** — tant que `answer` est `null`, le tour de l'assistant enchaîne l'attente puis les sources, qui est le comportement réel de l'app. Voir `docs/design-system.md` § 8.
...
1. **SSR page par page.** Seules les pages avec `export const prerender = false` interrogent l'API à la requête — c'est le cas de la page d'accueil depuis `mibeko-site#2` : ses compteurs (`fetchLibraryFundStats`) sont donc à jour à chaque requête, pas figés au build. Une page sans cette directive reste pré-rendue et ses appels API partent au build (instantané de l'image Docker jusqu'au redéploiement) — vérifier au cas par cas plutôt que de supposer l'accueil concerné.
...
5. **Conformité stores déjà subie** (rejet Play « Misleading Claims ») : **ne jamais promettre sur le site une capacité que l'app n'a pas**, et citer les sources officielles (sgg.cg, ohada.org) — déjà fait dans `/methode` et `/cgu`, à préserver. `/tarifs` et `/produits` n'existent plus (301 vers `/professionnels` et `/application`, cf. `astro.config.mjs`) et aucun prix n'est publié : `professionnels.astro` renvoie vers une démonstration. Ne pas réintroduire de tarif.
